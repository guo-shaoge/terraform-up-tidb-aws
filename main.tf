...
resource "aws_instance" "tiflash_wn" {
  count = local.n_tiflash_wn

  ami                         = local.image
  instance_type               = local.tiflash_wn_instance
  key_name                    = aws_key_pair.master_key.id
  vpc_security_group_ids      = [aws_security_group.ssh.id]
  subnet_id                   = aws_subnet.main.id
  associate_public_ip_address = true
  ...
  tags = {
    Name = "${local.name}-tiflash_wn-${count.index}"
  }
...
resource "aws_instance" "tiflash_rn" {
  count = local.n_tiflash_rn

  ami                         = local.image
  instance_type               = local.tiflash_rn_instance
  key_name                    = aws_key_pair.master_key.id
  vpc_security_group_ids      = [aws_security_group.ssh.id]
  subnet_id                   = aws_subnet.main.id
  associate_public_ip_address = true
  ...
  tags = {
    Name = "${local.name}-tiflash_rn-${count.index}"
  }
...
resource "aws_instance" "center" {
  ...
  provisioner "file" {
    content = templatefile("./files/topology.yaml.tftpl", {
      ...
      tiflash_wn_hosts = aws_instance.tiflash_wn.*.private_ip,
      tiflash_rn_hosts = aws_instance.tiflash_rn.*.private_ip,
    })
...
output "private-ip-tiflash_wn" {
  value = aws_instance.tiflash_wn.*.private_ip
}
output "private-ip-tiflash_rn" {
  value = aws_instance.tiflash_rn.*.private_ip
}
